...
["K/D zero kills and deaths is safe", ([0,9,0] call bn_koth_fnc_menu_formatCareerValue) isEqualTo "0.00"] call _check;
...
["Stats read uses the semantic career summary", (_requestSource find "call bn_koth_fnc_career_querySummary") >= 0] call _check;
private _summarySource = loadFile "functions\career\fn_querySummary.sqf";
["Career summary uses exact SQL_CUSTOM statement", (_summarySource find "[""loadCareerSummary"", [_uid]]") >= 0] call _check;
["Career summary never forces a flush", (_summarySource find "career_flushPlayer") < 0] call _check;
["Career summary has no per-metric fan-out", (_summarySource find "career_queryLeaderboard") < 0] call _check;
["Client request cannot supply a UID", (_clientRequestSource find "_uid") < 0] call _check;
["Server derives the requesting player from owner", (_requestSource find "remoteExecutedOwner") >= 0 && {(_requestSource find "bn_koth_fnc_teams_getPlayerByOwner") >= 0} && {(_requestSource find "getPlayerUID") >= 0}] call _check;
["Stats response targets only its requester", (_requestSource find "remoteExecCall [""bn_koth_fnc_menu_receiveStats"", _ownerId]") >= 0] call _check;
