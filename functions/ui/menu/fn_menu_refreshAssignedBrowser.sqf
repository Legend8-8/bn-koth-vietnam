...
{{(_display displayCtrl _x) ctrlShow false;} forEach _x;} forEach _cards;
{
    ...
    if (_stage isEqualTo 1) then {
        ...
        _button buttonSetAction (if !(_target isEqualTo "") then {format ["uiNamespace setVariable ['BN_KOTH_menuSelectorReturnPage','LOADOUT_ASSIGNED'];uiNamespace setVariable ['BN_KOTH_menuSelectorReturnBrowserPage',uiNamespace getVariable ['BN_KOTH_menuBrowserPage',0]];uiNamespace setVariable ['BN_KOTH_menuBrowserSlot','%1'];uiNamespace setVariable ['BN_KOTH_menuBrowserSnapPending',true];['LOADOUT_BROWSER'] call bn_koth_fnc_menu_refresh;",if (_target isEqualTo "LOADOUT_FACEWEAR") then {"facewear"} else {"binocular"}]} else {format ["uiNamespace setVariable ['BN_KOTH_menuAssignedStage',2];uiNamespace setVariable ['BN_KOTH_menuAssignedSlot',%1];uiNamespace setVariable ['BN_KOTH_menuBrowserSnapPending',true];['LOADOUT_BROWSER'] call bn_koth_fnc_menu_refresh;",_entry getOrDefault ["assignedIndex",-1]]});
    } else {
