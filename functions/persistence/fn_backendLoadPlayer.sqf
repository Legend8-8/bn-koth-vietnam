...
if (_backend isEqualTo "EXTDB3") exitWith {
    ...
    if (_savedKitsDropped) then {
        ...
        // corrupt kit blob fails closed to an empty set without discarding the
        // otherwise valid XP/cash/ownership record; the loaded state is marked
        // for a canonical rewrite by fn_loadPlayer.
        _savedKits = createHashMapFromArray [["success", true], ["kits", []], ["preferredId", ""]];
        [format ["Malformed saved-kit blob dropped while preserving progression UID=%1", _uid], "ERROR"] call bn_koth_fnc_common_log;
    };
    ...
    createHashMapFromArray [
        ...
        ["uid", _uid], ["found", true], ["record", _record], ["savedKitsDropped", _savedKitsDropped]
    ]
};
