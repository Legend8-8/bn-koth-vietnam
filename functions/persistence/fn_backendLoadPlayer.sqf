...
if (_backend isEqualTo "EXTDB3") exitWith {
    ...
    if !(_row isEqualType [] && {(count _row) isEqualTo 8}) exitWith {createHashMapFromArray [["success", false], ["code", "MALFORMED_LOAD_ROW"], ["uid", _uid]]};
    _row params ["_recordUid", "_schemaVersion", "_xp", "_cash", "_ownedText", "_killsText", "_ownedPerksText", "_activePerksText"];
    if !(_recordUid isEqualType "" && {_schemaVersion isEqualType 0} && {_xp isEqualType 0} && {_cash isEqualType 0} && {_ownedText isEqualType ""} && {_killsText isEqualType ""} && {_ownedPerksText isEqualType ""} && {_activePerksText isEqualType ""}) exitWith {createHashMapFromArray [["success", false], ["code", "MALFORMED_LOAD_TYPES"], ["uid", _uid]]};
