/* Author: Legend; Description: Changes one active personal paid vehicle's access mode after owner validation. Execution: Server; Public: No */
params [["_uid","",[""]],["_mode","",[""]]];
...
private _active=missionNamespace getVariable ["BN_KOTH_vehicleActivePersonal",createHashMap];private _record=_active getOrDefault [_uid,createHashMap];private _vehicle=_record getOrDefault ["vehicle",objNull];
if (isNull _vehicle || {!alive _vehicle}) exitWith {["NO_ACTIVE_VEHICLE","No active personal vehicle exists."] call _fail};
...
_record set ["accessMode",_mode];_active set [_uid,_record];missionNamespace setVariable ["BN_KOTH_vehicleActivePersonal",_active];_vehicle setVariable ["BN_KOTH_personalAccessMode",_mode,true];
