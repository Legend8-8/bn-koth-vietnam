...
if !(_newState in _validStates) exitWith {
    [format ["Rejected invalid round state: %1", _newState], "ERROR"] call bn_koth_fnc_log;
};

private _previousState = [] call bn_koth_fnc_round_getState;
if (_previousState isEqualTo _newState) exitWith {
    [format ["Round state unchanged: %1", _newState], "INFO"] call bn_koth_fnc_log;
};
...
if !(_newState in _allowedNext) exitWith {
    [format ["Rejected invalid transition: %1 -> %2", _previousState, _newState], "ERROR"] call bn_koth_fnc_log;
};
...
switch (_newState) do {
    case "PREPARING": {
        ...
        ["BN_KOTH_prepareEndAt", serverTime + _prepareDuration] call bn_koth_fnc_publicState;
    ...
    case "ENDING": {
        ...
        ["BN_KOTH_endingEndAt", serverTime + _endingDuration] call bn_koth_fnc_publicState;
