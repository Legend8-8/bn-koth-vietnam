...
if !(_state isEqualTo "RESETTING") exitWith {
	[format ["Ignored reset outside RESETTING state (%1)", _state], "WARN"] call bn_koth_fnc_log;
	false
};

private _resetDuration = missionNamespace getVariable ["BN_KOTH_resetDuration", 5];
["BN_KOTH_resetEndAt", serverTime + _resetDuration] call bn_koth_fnc_publicState;
...
["BN_KOTH_zonePopulation", [0, 0]] call bn_koth_fnc_publicState;
