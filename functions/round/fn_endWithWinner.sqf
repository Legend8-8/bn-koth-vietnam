...
if !(_state isEqualTo "ACTIVE") exitWith {
    [format ["Ignored winner declaration in state %1", _state], "WARN"] call bn_koth_fnc_common_log;
    false
};

if !([_winningSide] call bn_koth_fnc_teams_validateSide) exitWith {
    [format ["Rejected invalid winning side: %1", _winningSide], "ERROR"] call bn_koth_fnc_common_log;
    false
};
